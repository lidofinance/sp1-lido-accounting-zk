...
use sp1_lido_accounting_scripts::eth_client::ProviderFactory;
...
struct ScriptConfig {
    ...
    write_test_fixture: Option<Path>,
}
...
impl ScriptSteps {
    ...
    pub fn verify_input_correctness(&self, program_input: &ProgramInput, public_values: &PublicValuesRust) {
        if !self.config.verify_input {
            log::info!("Skipping verifying input");
            return Ok(());
        }
        ...
        log::info!("Public values verified");
    }
    ...
    pub fn write_test_fixture(&self, proof: &SP1ProofWithPublicValues) {
        if self.config.write_test_fixture.is_none() {
            return;
        }
        ...
        std::fs::create_dir_all(&fixture_file.parent()).expect("failed to create fixture path");
...
fn prove(steps: ScriptSteps, program_input: &ProgramInput, expected_public_values: &PublicValuesRust) {
    steps.verify_input_correctness(program_input, public_values);
...
async fn main() {
    ...
    let script_config = ScriptConfig {
        ...
        write_test_fixture: write_fixture,
    };
